...
namespace FastShooter
{
    public class Player2 : Player
    {
        ...
        private List<Keys> keyList = new List<Keys>(new Keys[] { Keys.Up, Keys.Down ,Keys.Left, Keys.Right, Keys.M, Keys.N });
        ...
        protected override void Movement()
        {
            ...
            if (keyboardMode == true)
            {
                ...
                if (PolyInput.Keyboard.Pressed(Keys.M))
                {
                    if (counters.Check("dodgeCoolTimer") == false)
                    {
                        dodgeStart = true;
                ...
                else if (PolyInput.Keyboard.Released(Keys.M))
                {
                    counters["dodgeTimer"] = 0.0f;
                    PlayerColour = assignColour;
                }
                ...
                else if (PolyInput.Keyboard.Check(Keys.Down) == true) {
                    signY = 1;
                }

                if (PolyInput.Keyboard.Check(Keys.Right) == true) {
                    signX = 1;
                }
                else if (PolyInput.Keyboard.Check(Keys.Left) == true) {
                    signX = -1;
                }
                else if (counters.Check("dodgeTimer") == false) {
                    Velocity.X = 0;
        ...
        protected override void Shooting()
        {
            ...
            if (keyboardMode == true)
            {
                if (PolyInput.Keyboard.Pressed(Keys.N) == true)
                {
                    timerSet = true;
                    isShooting = true;
                    counters["resetAnimation"] = 100.0f;
                ...
                else if (PolyInput.Keyboard.Check(Keys.N) == true && timerSet == true)
                {
                    isShooting = true;
                    counters["resetAnimation"] = 100.0f;
                    ...
                    if (counters.Check("chargeTimer2") == false && timerSet == true) {
                        timerSet = false;
                        charged1 = false;
                        charged2 = true;
                ...
                if (PolyInput.Keyboard.Released(Keys.N) == true) {
                    counters["resetAnimation"] = 100.0f;
                    BulletCreationLevel2();
        ...
        public override void HealthBarDrawing()
        {
            ...
            int damageDealt = 100 - (int)playerHealth;
            healthRectangle = new Rectangle(damageDealt, 0, (int)playerHealth, healthRectangle.Height);
            
            if(PlayerHit == true)
            {
                if(lastHitDamage == 0) {
                    healthBarPosition.X += damageDealt;
                }
                else {
                    healthBarPosition.X += damageDealt - lastHitDamage;
                }
            ...
            Engine.SpriteBatch.Draw(healthBarImage2, healthBarPosition, healthRectangle, Color.White);
        }
